...
export default function AppGate() {
  ...
  const [faceBusy,setFaceBusy]=useState(false), [faceMsg,setFaceMsg]=useState(""), [offerFace,setOfferFace]=useState(false);
  const [accounts,setAccounts]=useState([]), [editor,setEditor]=useState(null), [busy,setBusy]=useState(false), [appVersion,setAppVersion]=useState(0);

  useEffect(() => installReceiptOcrFetchInterceptor(), []);
  useEffect(() => { let alive=true; (async()=>{ try { const {data}=await supabase.auth.getSession(); if(data?.session) await supabase.auth.signOut({scope:"local"}); } catch(e){ console.error(e); } finally { if(alive)setBooting(false); } })(); return()=>{alive=false}; }, []);
